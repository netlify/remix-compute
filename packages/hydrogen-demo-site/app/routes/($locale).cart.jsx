...
export async function action({ request, context }) {
  ...
  const countryCode = formData.get('countryCode') ? formData.get('countryCode') : null
  ...
  switch (cartAction) {
    ...
    case CartAction.REMOVE_FROM_CART:
      ...
      const lineIds = formData.get('linesIds') ? JSON.parse(String(formData.get('linesIds'))) : []
    ...
    case CartAction.UPDATE_CART:
      invariant(cartId, 'Missing cartId')
      const updateLines = formData.get('lines') ? JSON.parse(String(formData.get('lines'))) : []
    ...
    case CartAction.UPDATE_BUYER_IDENTITY:
      const buyerIdentity = formData.get('buyerIdentity') ? JSON.parse(String(formData.get('buyerIdentity'))) : {}
  ...
  return (
    <div className="grid w-full gap-8 p-6 py-8 md:p-8 lg:p-12 justify-items-start">
      <Suspense fallback={<CartLoading />}>
        <Await resolve={root.data?.cart}>{(cart) => <Cart layout="page" cart={cart} />}</Await>
      </Suspense>
    </div>
  )
}
...
export async function cartRemove({ cartId, lineIds, storefront }) {
  const { cartLinesRemove } = await storefront.mutate(REMOVE_LINE_ITEMS_MUTATION, {
    variables: {
      cartId,
      lineIds,
    },
 ...
 * @preserve
 */
export async function cartUpdateBuyerIdentity({ cartId, buyerIdentity, storefront }) {
  const { cartBuyerIdentityUpdate } = await storefront.mutate(UPDATE_CART_BUYER_COUNTRY, {
    variables: {
      cartId,
      buyerIdentity,
    },
 ...
 * @preserve
 */
export async function cartDiscountCodesUpdate({ cartId, discountCodes, storefront }) {
  const { cartDiscountCodesUpdate } = await storefront.mutate(DISCOUNT_CODES_UPDATE, {
    variables: {
      cartId,
      discountCodes,
    },
