...
import { Button, Heading, IconRemove, Text, Link, FeaturedProducts } from '~/components'
    ...
    <>
      ...
      <UpdateDiscountForm>
        <div className={clsx(codes ? 'hidden' : 'flex', 'items-center gap-4 justify-between text-copy')}>
          <input className={getInputStyleClasses()} type="text" name="discountCode" placeholder="Discount code" />
          <button className="flex justify-end font-medium whitespace-nowrap">Apply Discount</button>
        </div>
      </UpdateDiscountForm>
    </>
  )
...
function ItemRemoveButton({ lineIds }) {
  ...
  return (
    <fetcher.Form action="/cart" method="post">
      ...
      <button className="flex items-center justify-center w-10 h-10 border rounded" type="submit">
...
export function CartEmpty({ hidden = false, layout = 'drawer', onClose }) {
  ...
  const container = {
    ...
    page: clsx([hidden ? '' : 'grid', `pb-12 w-full md:items-start gap-4 md:gap-8 lg:gap-12`]),
  }
