...
import { Link, useLocation, useSearchParams, useNavigate } from '@remix-run/react'
import { useDebounce } from 'react-use'
import { Disclosure } from '@headlessui/react'

import { Heading, IconFilters, IconCaret, IconXMark, Text } from '~/components'

export function SortFilter({ filters, appliedFilters = [], children, collections = [] }) {
...
export function FiltersDrawer({ filters = [], appliedFilters = [], collections = [] }) {
  ...
  const filterMarkup = (filter, option) => {
    switch (filter.type) {
      ...
      default:
        ...
        return (
          <Link className="focus:underline hover:underline" prefetch="intent" to={to}>
            {option.label}
          </Link>
        )
    }
...
export default function SortMenu() {
  ...
  return (
    <Menu as="div" className="relative z-40">
      ...
      <Menu.Items as="nav" className="absolute right-0 flex flex-col p-4 text-right rounded-sm bg-contrast">
        {items.map((item) => (
